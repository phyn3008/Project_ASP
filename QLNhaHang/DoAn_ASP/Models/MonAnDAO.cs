...
namespace DoAn_ASP.Models
{
    public class MonAnDAO
    {
        ...
        // Sửa món ăn cũ
        public int Update(string mama, string mapl, string tenmonan, string dongia, string hinhanh, string mieuta, Boolean cosan)
        {
            //1. mở kết nối 
            SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["QLNH_ASPConnectionString"].ConnectionString);
            conn.Open();
            ...
            //2. Tạo truy vấn
            SqlCommand cmd = new SqlCommand("Update Menu set mapl=@mapl, tenma=@tenma, dongiasp=@dongia, hinhanh=@hinhanh, mieuta=@mieuta, cosan=@cosan whewe mama=@mama", conn);
            cmd.Parameters.AddWithValue("@mama", mama);
            cmd.Parameters.AddWithValue("@mapl", mapl);
            cmd.Parameters.AddWithValue("@tenma", tenmonan);
            cmd.Parameters.AddWithValue("@dongia", dongia);
